...
void setup_pointcloud_1(std::vector<jmk::Point2d>& points)
{
...
{
	points.push_back(jmk::Point2d(-0.5, 0.0));
	points.push_back(jmk::Point2d(-0.4, 0.4));
	points.push_back(jmk::Point2d(0.0, 0.7));
	points.push_back(jmk::Point2d(0.0, 0.4));
	points.push_back(jmk::Point2d(0.4, 0.4));
	points.push_back(jmk::Point2d(0.0, 0.0));
	points.push_back(jmk::Point2d(0.9, 0.0));
	points.push_back(jmk::Point2d(0.4, -0.4));
	points.push_back(jmk::Point2d(0.0, -0.5));
	points.push_back(jmk::Point2d(-0.4, -0.4));
	//add more random points in x=[-0.5, 0.5], y=[-0.2, 0.8]
	{
		size_t seeds = 100;
		size_t maxnum = 100;
		while (seeds-->0)
		{
			float x = (std::rand() % maxnum) / (maxnum * 1.0) - 0.5;
			float y = (std::rand() % maxnum) / (maxnum * 1.0) - 0.2;
			points.push_back(jmk::Point2d(x, y));
		}
	}
}
int main(void)
{	
	...
	endTime = glfwGetTime();
	std::cout <<"convexhull2DGiftwrapping: "<< endTime - beginTime << std::endl;
	...
	endTime = glfwGetTime();
	std::cout << "convexhull2DModifiedGrahams: " << endTime - beginTime << std::endl;
	...
	beginTime = glfwGetTime();
	jmk::convexhull2DSlow(points, hull_points_slow);
	endTime = glfwGetTime();
	std::cout << "convexhull2DSlow: " << endTime - beginTime << std::endl;
	jmk::Polygon2d polygon3(hull_points_slow);
	edge_list = polygon3.getEdgeList();
	get2DLinePointsFromDCEL2d(edge_list, convexhull_slow_edge_data);

	std::vector<float> point_axis{-1, 0, 1, 0, 0, -1, 0, 1};
	VertexArray VAO_Axis;
	VertexBuffer VBO_Axis(point_axis.data(), point_axis.size());
	VAO_Axis.addVertexLayout(0, 2, GL_TRUE, 2 * sizeof(float), 0);
	...
	VertexArray VAO_convexhullslow;
	VertexBuffer VBO_edge_slow(convexhull_slow_edge_data.data(), convexhull_slow_edge_data.size());
	VAO_convexhullslow.addVertexLayout(0, 2, GL_FALSE, 2 * sizeof(float), 0);

	ShaderProgram shader("D:/D/Mastering-Computational-Geomentry/Graphics/GraphicUtils/Shaders/triangle2d.shader");
	ShaderProgram line_shader("D:/D/Mastering-Computational-Geomentry/Graphics/GraphicUtils/Shaders/generic_line.shader");
	...
	bool show_points = true, show_convexhull_giftwrapping = false, show_convexhull_grahams = false, show_convexhull_slow = false;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (show_points)
		{
			...
			VAO_Axis.bindVertexArray();
			glDrawArrays(GL_LINES, 0, point_axis.size() / 2);
		}
		...
		if (show_convexhull_slow) {
			line_shader.activeAsCurrentShader();
			glLineWidth(4);
			glUniform3fv(line_color_loc, 1, glm::value_ptr(blue));
			VAO_convexhullslow.bindVertexArray();
			glDrawArrays(GL_LINES, 0, convexhull_slow_edge_data.size() / 2);
		}
		...
		ImGui::Checkbox("Show Convexhull Slow", &show_convexhull_slow);
